...
class ServicoClient {
  ...
  Future<Either<ErrorEntity, PageContent<Servico>>> getServicosByFilter(
    ...
    try {
      final jsonToSend = {
        ...
        'marca': servicoFilter.marca,
        'situacao': servicoFilter.situacao != null
            ? Formatters.mapSituationToEnumSituation(servicoFilter.situacao!)
            : null,
        'garantia': servicoFilter.garantia,
        'filial': servicoFilter.filial,
        'periodo': servicoFilter.periodo,
        'dataAtendimentoPrevistoAntes': servicoFilter
            .dataAtendimentoPrevistoAntes
            ?.toIso8601String(),
        'dataAtendimentoPrevistoDepois': servicoFilter
            .dataAtendimentoPrevistoDepois
            ?.toIso8601String(),
        'dataAtendimentoEfetivoAntes': servicoFilter.dataAtendimentoEfetivoAntes
            ?.toIso8601String(),
        'dataAtendimentoEfetivoDepois': servicoFilter
            .dataAtendimentoEfetivoDepois
            ?.toIso8601String(),
        'dataAberturaAntes': servicoFilter.dataAberturaAntes?.toIso8601String(),
        'dataAberturaDepois': servicoFilter.dataAberturaDepois
            ?.toIso8601String(),
      };
      ...
      if (response.data != null && response.data is Map<String, dynamic>) {
        return Right(
          PageContent.fromJson(response.data, (json) => Servico.fromJson(json)),
        );
      }
    } on DioException catch (e) {
      return Left(ErrorHandler.onRequestError(e));
  ...
  Future<Either<ErrorEntity, void>> disableListOfServico(
