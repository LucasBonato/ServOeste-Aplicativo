...
class ReportService {
  ...
  Future<void> generate({required ReportType type, required Servico servico, required Cliente cliente}) {
    return Tracing.trace(
      'report.generate',
      attributes: {'report.type': type.name},
      fn: () => _generate(type: type, servico: servico, cliente: cliente),
    );
  }

  Future<void> _generate({required ReportType type, required Servico servico, required Cliente cliente}) async {
    ...
    return Tracing.trace(
      ...
      fn: () async {
        try {
          ...
          AppLogger.exception(e, stackTrace: stackTrace);
          return [];
        }
      },
    );
