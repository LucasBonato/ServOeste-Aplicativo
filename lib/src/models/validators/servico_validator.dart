...
class ServicoValidator extends LucidValidator<ServicoForm>
  ...
  void _initializeRules() {
    ...
    ruleFor((servico) => servico.dataAtendimentoPrevisto.value,
            key: ErrorCodeKey.dataAtendimentoPrevisto.name)
        .when((servico) =>
            _shouldValidateField(ErrorCodeKey.dataAtendimentoPrevisto.name) &&
            servico.situacao.value != "Cancelado")
        ...
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.dataAtendimentoPrevisto.name);
    ...
    ruleFor((servico) => servico.valor.value, key: ErrorCodeKey.valor.name)
        ...
        .must((valor) => valor != "", "Valor do serviço é obrigatório",
            ErrorCodeKey.valor.name)
        .customValidExternalErrors(externalErrors, ErrorCodeKey.valor.name);

    ruleFor((servico) => servico.valorPecas.value,
            key: ErrorCodeKey.valorPecas.name)
        .when((servico) =>
        ...
        .must((valorPecas) => valorPecas != "", "Valor das peças é obrigatório",
            ErrorCodeKey.valorPecas.name)
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.valorPecas.name);

    ruleFor((servico) => servico.dataAtendimentoEfetivo.value,
            key: ErrorCodeKey.dataAtendimentoEfetivo.name)
        .when((servico) =>
        ...
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.dataAtendimentoEfetivo.name);

    ruleFor((servico) => servico.dataInicioGarantia.value,
            key: ErrorCodeKey.dataInicioGarantia.name)
        .when((servico) =>
        ...
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.dataInicioGarantia.name);

    ruleFor((servico) => servico.dataFinalGarantia.value,
            key: ErrorCodeKey.dataFinalGarantia.name)
        .when((servico) =>
        ...
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.dataFinalGarantia.name);

    ruleFor((servico) => servico.formaPagamento.value,
            key: ErrorCodeKey.formaPagamento.name)
        .when((servico) =>
        ...
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.formaPagamento.name);

    ruleFor((servico) => servico.horario.value, key: ErrorCodeKey.horario.name)
        .when((servico) =>
            _shouldValidateField(ErrorCodeKey.horario.name) &&
            servico.situacao.value != "Cancelado")
        .must((horario) => horario != "", "Selecione um horário!",
            ErrorCodeKey.horario.name)
        .customValidExternalErrors(externalErrors, ErrorCodeKey.horario.name);

    ruleFor((servico) => servico.dataPagamentoComissao.value,
            key: ErrorCodeKey.dataPagamentoComissao.name)
        .when((servico) =>
            _shouldValidateField(ErrorCodeKey.dataPagamentoComissao.name) &&
            isUpdate)
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.dataPagamentoComissao.name);

    ruleFor((servico) => servico.dataFechamento.value,
            key: ErrorCodeKey.dataFechamento.name)
        .when((servico) =>
        ...
        .customValidExternalErrors(
            externalErrors, ErrorCodeKey.dataFechamento.name);
  }
