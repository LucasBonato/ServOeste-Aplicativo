...
class TecnicoValidator extends LucidValidator<TecnicoForm>
  ...
  TecnicoValidator({this.isUpdate = false}) {
    ...
    ruleFor((tecnico) => tecnico.situacao.value,
        ...
        .must((situacao) => ((isUpdate) ? situacao.isNotEmpty : true),
            "Selecione uma situação.", ErrorCodeKey.situacao.name);

    ruleFor((tecnico) => tecnico.telefoneCelular.value,
            key: ErrorCodeKey.telefoneCelular.name)
        .must(
            (celular) =>
                ((celular.isNotEmpty && celular.length < 15) ? false : true),
            "Tamanho inválido do telefone celular!",
    ...
    ruleFor((tecnico) => tecnico.telefoneFixo.value,
            key: ErrorCodeKey.telefoneFixo.name)
        .must(
            (fixo) => ((fixo.isNotEmpty && fixo.length < 14) ? false : true),
            "Tamanho inválido do telefone fixo!",
